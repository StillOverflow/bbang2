         ...
         <div class="card">
            ...
            <div class="card-body p-4">
               ...
               <div class="text-right mb-2">
                  <button class="btn btn-outline-success" @click="excelDownloadHeader"><i class="fa-regular fa-file-excel"></i> EXCEL</button>
               </div>
               <ag-grid-vue
                  class="ag-theme-alpine"
                  style="width: 100%; height: 500px;"
                  :rowData="filterRowData"
                  :gridOptions="materialOrderOptions"
                  @grid-ready="materialOrderGrid"
                  @firstDataRendered="materialOrderGridRendered"
            ...
            <div class="card-body p-4">
               <!-- 엑셀 다운로드 -->
               <div class="text-right mb-2">
                  <button class="btn btn-outline-success" @click="excelDownloadDetail"><i class="fa-regular fa-file-excel"></i> EXCEL</button>
               </div>

               <ag-grid-vue
                  class="ag-theme-alpine"
                  style="width: 100%; height: 500px;"
                  :rowData="filterDtlRowData"
                  :gridOptions="materialOrderDtlOptions"
                  @grid-ready="materialOrderDtlGrid"
                  @firstDataRendered="materialOrderDtlGridRendered"
                  @grid-size-changed="onGridSizeChanged"
               />
            </div>
         </div>
...
//! ---------------------------------------- 데이터 정의 ----------------------------------------
   ...
   let materialOrderData = shallowRef([]);  // 자재 발주서 헤더 데이터
   let materialOrderDtlData = shallowRef([]); // 자재 발주서 디테일 데이터

   let accountGridRendered = ref(null);     // 거래처 그리드
   let materialGridRendered = ref(null);    // 자재 그리드
   let materialOrderGridRendered = ref(null); // 자재 발주서 헤더

//! ---------------------------------------- Vue Hook ----------------------------------------
   // created와 비슷~~
   onBeforeMount(() => {
      store.dispatch('breadCrumb', { title: '자재 발주서 조회' });  // 페이지 제목 설정
      getCategory();  // 카테고리
      getType();      // 자재유형 

      getMaterialOrderList(); // 자재 재고 조회
   });
...
//! ----------------------------------- Event Handler -----------------------------------
   ...
   let filterDtlRowData = ref([]);           // 필터한 데이터 담을곳
...
//! ---------------------------------------- Ag Grid ----------------------------------------   
   ...
   // 자재 입고조회 그리드
   const materialOrderGrid = (params) =>{
      materialOrderGridRendered.value = params.api; // API 객체 저장
   ...
   const materialOrderOptions = {
      ...
      onRowClicked : (event) => {
         ...
         matKeyword.value = event.data.mat_order_cd
   ...
   // 자재 발주서 디테일
   const materialOrderDtlOptions = {
      columnDefs : [
         {
            headerName: '발주서 디테일 코드',
            field: 'mat_order_dtl_cd',
            cellClass: "text-center",
            cellRenderer: (params) => {
               // 렌더링 시 값이 없을 경우 표시
               return params.value ? params.value : `<span style="color: #cacaca; font-size: 11px">데이터없음</span>`;
   ...
   const excelDownloadHeader = () => {
      ...
      const header = materialOrderOptions.columnDefs.map((col) => col.headerName);
      const data = filterRowData.value.map((row) =>
      materialOrderOptions.columnDefs.map((col) => {
      ...
      // 엑셀 파일 다운로드
      XLSX.writeFile(workBook, `자재_발주서헤더_${new Date().toISOString().slice(0, 10)}.xlsx`);
   };

   // 디테일 테이블 엑셀 다운로드
   const excelDownloadDetail = () => {

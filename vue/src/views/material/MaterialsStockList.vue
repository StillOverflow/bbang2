   ...
   <div class="py-4 container-fluid">
      <div class="row" @keydown="keyEventHandler($event)">
         ...
         <div class="card">
            ...
            <div class="card-body p-4"><!-- //& 자재 모달창 -->
...
//! ---------------------------------------- 데이터 정의 ----------------------------------------
   ...
   let materialStockData = shallowRef([]);

   let materialGridRendered = ref(null);    // 자재 그리드
   let materialStockGridRendered = ref(null);
...
//! ---------------------------------------- axios 서버통신 ----------------------------------------
   ...
   const getStockList = async () => {
      try {
         const result = await axios.get('/api/material/stockList' );
         materialStockData.value = result.data || [];
         filterRowData.value = materialStockData.value

      } catch (err) {
         materialStockData.value = [];

...
export default {
  ...
  created() {
    // 페이지 제목 저장
    this.$store.dispatch('breadCrumb', { title: '설비 상태 조회' });
    // 공통코드 및 초기 데이터 가져오기
  ...
  methods: {
    ...
    async fetchFilteredEquip() {
      try {
        ...
        console.error('설비 상태 데이터 조회 실패:', error);
      }
    },
    // 조회 버튼 클릭 시 실행
    searchEquipments() {
    ...
    excelDownload() {
      try {
        ...
        XLSX.utils.book_append_sheet(workBook, workSheet, '설비상태조회');
        XLSX.writeFile(workBook, `설비상태조회_${today}.xlsx`);
      } catch (error) {

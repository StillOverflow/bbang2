  ...
  data() {
    return {
      ...
      columnDefs: [
        ...
        { field: 'next_insp_dt', headerName: '차기점검일', sortable: true, valueFormatter: this.$comm.dateFormatter_returnNull, cellStyle: { textAlign: 'center' }, headerClass: 'ag-header-center', },
      ...
      try {
        ...
        if (selectedNodes.length > 0) {
          ...
          selectedData = selectedNodes.map(item => ({
            ...
            '설비구분': item?.eqp_type,
            '설비명': item?.eqp_nm,
            '점검주기(일)': item?.insp_cycle ? item.insp_cycle.toString() : '', // 문자열로 변환
